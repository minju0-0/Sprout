...
describe("catchUpSeason", () => {
  ...
  it("harvests exactly one season when the calendar has moved by one month", () => {
    const categories = [makeCategory({ spent: 90 })];
    const now = new Date(2026, 7, 1);
    const result = catchUpSeason("July 2026", categories, [], now);
    expect(result.rolledOver).toBe(true);
    expect(result.activeSeason).toBe("August 2026");
    expect(result.categories.every((category) => category.spent === 0)).toBe(true);
  ...
  it("harvests one record per skipped month, most recent first, when multiple months were missed", () => {
    const categories = [makeCategory({ spent: 50 })];
    ...
    expect(result.harvestHistory[1].categories[0].spent).toBe(0);
  });
